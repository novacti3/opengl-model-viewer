...
void Renderer::DrawMesh(Mesh* const mesh, Shader* const shader)
{
    ...
    GL_CALL(glad_glBindVertexArray(mesh->getVAO()));
    shader->Bind();
    GL_CALL(glad_glDrawArrays(GL_TRIANGLES, 0, mesh->getVertices().size()));
    shader->Unbind();
    GL_CALL(glad_glBindVertexArray(0));
}
